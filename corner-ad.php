    ...
    function corner_ad_replace_shortcode($attr){
        ...
        if(isset($attr['id']) && !isset($corner_ad_inserted)){
            ...
            if($ad){
                ...
                $colorIn = $ad->colorIn;
				return "<script>if(window.addEventListener){ window.addEventListener('load', function(){ printCornerAd({alignTo:'tl', mirror:".(($ad->mirror == 1) ? 'true' : 'false').", colorIn:'".$colorIn."', thumbPath:'".$img->thumb_url."', imgPath:'".$img->url."', adUrl:'".$ad->adURL."', openIn:".(($ad->openIn) ? $ad->openIn : -1).", closeIn:".(($ad->closeIn) ? $ad->closeIn : -1).", target:'".$ad->target."'}); }); }else{ window.attachEvent('onload', function(){ printCornerAd({alignTo:'tl', mirror:".(($ad->mirror == 1) ? 'true' : 'false').", colorIn:'".$colorIn."', thumbPath:'".$img->thumb_url."', imgPath:'".$img->url."', adUrl:'".$ad->adURL."', openIn:".(($ad->openIn) ? $ad->openIn : -1).", closeIn:".(($ad->closeIn) ? $ad->closeIn : -1).", target:'".$ad->target."'}); }); } </script>";
            }    
        }    
...
if(!function_exists('corner_ad_media_button')){
    function corner_ad_media_button(){
        ...
        if($ads){
            foreach($ads as $ad){
                $list .= '<option value="'.$ad->id.'">'.$ad->name.'</option>';
            }
        }
        
        wp_localize_script('corner_ad_insertion', 'corner_ad', array('list' => $list));
        
        print '<a href="javascript:open_insertion_corner_ad_window();" title="'.__('Insert Corner Ad').'"><img src="'.CORNER_AD_PLUGIN_URL.'/images/corner-ad-icon.gif'.'" alt="'.__('Insert Corner Ad').'" /></a>';
